...
import { geminiAPI, textToSpeech } from "@/utils/service";
import { useRouter } from "next/navigation";
import { useEffect, useState } from "react";
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { faMicrophone } from '@fortawesome/free-solid-svg-icons'

const Microphone = ({ chatMessages, setChatMessages, disabled=false }) => {
  ...
  useEffect(() => {
    if (text) {
      setChatMessages([...chatMessages, { role: "user", parts: [{ text: text }] }]);
      run();
    }
  }, [text]);
  ...
  return (
    <div className="flex flex-col gap-y-2 justify-center items-center">
      ...
      >
        <div className={"flex items-center justify-center bg-red-500 text-white rounded-full p-4 w-20 h-20 shrink-0  " + (recording && " microphone-animation") + (recordingState && "  animate-pulse") + (loading && " opacity-40")}>
          {!loading && <FontAwesomeIcon icon={faMicrophone} size="3x" />}
        </div>
        {loading && <div className="absolute flex justify-center items-center h-full">
          <div role="status">
